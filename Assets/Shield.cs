...
public class Shield : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.transform.tag == "EnemyBullet" || collision.transform.tag == "Bullet Pacman")
        {
            ...
            Vector3 eulerAngle = new Vector3(collision.transform.GetComponent<Transform>().rotation.eulerAngles.x, collision.transform.GetComponent<Transform>().rotation.eulerAngles.y, collision.GetComponent<Transform>().rotation.eulerAngles.z);
